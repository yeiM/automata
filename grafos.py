...
class automataGrafico:
	...
	def generarAutomata(self, mostrar, especial, nodo):
		...
		if nodo == "p":
			g.node("p", color = "#8245a0", shape='circle')
			g.node("q", shape='circle')
			g.node("r")
		elif nodo == "q":
			g.node("p", shape='circle')
			g.node("q", color = "#8245a0", shape='circle')
			g.node("r")
		elif nodo == "r":
			g.node("p", shape='circle')
			g.node("q", shape='circle')
			g.node("r", color="#8245a0")
		else:
			g.node("p", shape='circle')
			g.node("q", shape='circle')
			g.node("r")
		...
		if origen == "p" and destino == "p":
			g.edge("p", "p", label=self.__labelString(self.transiciones[0]), color="#8245a0")
		else:
			g.edge("p", "p", label=self.__labelString(self.transiciones[0]))

		if origen == "p" and destino == "q":
			g.edge("p", "q", label=self.__labelString(self.transiciones[1]), color="#8245a0")	
		else:
			g.edge("p", "q", label=self.__labelString(self.transiciones[1]))

		if origen == "q" and destino == "q":
			g.edge("q", "q", label=self.__labelString(self.transiciones[2]), color="#8245a0")
		else:
			g.edge("q", "q", label=self.__labelString(self.transiciones[2]))

		if origen == "q" and destino == "r":
			g.edge("q", "r", label=self.__labelString(self.transiciones[3]), color="#8245a0")
		else:
